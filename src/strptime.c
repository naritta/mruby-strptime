...
static const char *merid_names[] = {"am", "pm", "a.m.", "p.m."};

static const char *extz_pats[] = {":z", "::z", ":::z"};
#define sizeof_array(o) (sizeof o / sizeof o[0])

#define issign(c) ((c) == '-' || (c) == '+')

static long read_digits(mrb_state *mrb, const char *s, mrb_value *n,
                        size_t width) {
  size_t l;
...
static int valid_range_p(mrb_state *mrb, mrb_value v, int a, int b) {
  if (mrb_fixnum_p(v)) {
    int vi = mrb_fixnum(v);
    return !(vi < a || vi > b);
  }
  const char c = '<';
  const char d = '>';
  return !(mrb_bool(mrb_funcall(mrb, v, &c, 1, mrb_fixnum_value(a))) ||
           mrb_bool(mrb_funcall(mrb, v, &d, 1, mrb_fixnum_value(b))));
}

#define recur(fmt)                                                             \
  {                                                                            \
    size_t l;                                                                  \
    l = date__strptime_internal(mrb, &str[si], slen - si, fmt, sizeof fmt - 1, \
                                hash);                                         \
    si += l;                                                                   \
  }
...
static size_t date__strptime_internal(mrb_state *mrb, const char *str,
                                      size_t slen, const char *fmt, size_t flen,
                                      mrb_value hash) {
    ...
    again:
      ...
      case 'a': {
        ...
        for (i = 0; i < (int)sizeof_array(day_names); i++) {
          ...
          if (strncasecmp(day_names[i], &str[si], l) == 0) {
            si += l;
            mrb_hash_set(mrb, hash, mrb_str_new_cstr(mrb, "wday"),
                         mrb_fixnum_value(i % 7));
            goto matched;
          }
        }
        fail();
      ...
      case 'h': {
        ...
        for (i = 0; i < (int)sizeof_array(month_names); i++) {
          ...
          if (strncasecmp(month_names[i], &str[si], l) == 0) {
            si += l;
            mrb_hash_set(mrb, hash, mrb_str_new_cstr(mrb, "mon"),
                         mrb_fixnum_value((i % 12) + 1));
            goto matched;
          }
        }
        fail();
      ...
      case 'g': {
        ...
        if (mrb_nil_p(mrb_hash_get(mrb, hash, mrb_str_new_cstr(mrb, "_cent"))))
          mrb_hash_set(
              mrb, hash, mrb_str_new_cstr(mrb, "_cent"),
              mrb_fixnum_value(
                  mrb_bool(mrb_funcall(mrb, n, ">=", 1, mrb_fixnum_value(69)))
                      ? 19
                      : 20));
        goto matched;
      }
      ...
      case 'N': {
        ...
        if (sign == -1)
          mrb_funcall(mrb, n, "-@", 0);
        // set_hash("sec_fraction", rb_rational_new2(n, f_expt(INT2FIX(10),
        // ULONG2NUM(si - osi))));
        mrb_hash_set(mrb, hash, mrb_str_new_cstr(mrb, "sec_fraction"),
                     mrb_str_new_cstr(mrb, "unsupported"));
        goto matched;
      }
      ...
      case 'p': {
        ...
        for (i = 0; i < 4; i++) {
          ...
          if (strncasecmp(merid_names[i], &str[si], l) == 0) {
            si += l;
            mrb_hash_set(mrb, hash, mrb_str_new_cstr(mrb, "_merid"),
                         mrb_fixnum_value((i % 2) == 0 ? 0 : 12));
            goto matched;
          }
        }
        fail();
      ...
      case 'Q': {
        ...
        if (sign == -1)
          mrb_funcall(mrb, n, "-@", 0);
        // set_hash("seconds", rb_rational_new2(n, f_expt(INT2FIX(10),
        // INT2FIX(3))));
        mrb_hash_set(mrb, hash, mrb_str_new_cstr(mrb, "wday"),
                     mrb_str_new_cstr(mrb, "unsupported"));
        goto matched;
      ...
      case 'W': {
        ...
        mrb_hash_set(mrb, hash,
                     mrb_str_new_cstr(mrb, c == 'U' ? "wnum0" : "wnum1"), n);
        goto matched;
      }
      ...
      case 'z': {
        ...
        mrb_value obj =
            mrb_funcall(mrb, onig_value, "new", 2, pat, mrb_fixnum_value(1));
...
static mrb_value parse_method(mrb_state *mrb, mrb_value self) {
...
static mrb_value strptime_initialize(mrb_state *mrb, mrb_value self) {
  ...
  mrb_value offset =
      mrb_hash_get(mrb, time_hash, mrb_str_new_cstr(mrb, "offset"));
  mrb_iv_set(mrb, self, mrb_intern_lit(mrb, "@offset"), offset);

  return self;
}

static mrb_value strptime_to_time_method(mrb_state *mrb, mrb_value self) {
  mrb_value time_hash =
      mrb_iv_get(mrb, self, mrb_intern_lit(mrb, "@time_hash"));
  ...
  if (mrb_nil_p(year)) {
    ...
    time =
        mrb_funcall(mrb, time_obj, "new", 6, year, mon, mday, hour, min, sec);
  }

  return time;
}

static mrb_value strptime_to_i_method(mrb_state *mrb, mrb_value self) {
  ...
  if (!mrb_nil_p(offset)) {
    mrb_value ope = mrb_funcall(mrb, offset, "slice!", 2, mrb_fixnum_value(0),
                                mrb_fixnum_value(1));
    if (strcmp(RSTRING_PTR(ope), "+") && strcmp(RSTRING_PTR(ope), "-")) {
      return time_int_without_offset;
    }
    mrb_value offset_array =
        mrb_funcall(mrb, offset, "split", 1, mrb_str_new_cstr(mrb, ":"));
    ...
    mrb_value offset_hour_i_sec =
        mrb_funcall(mrb, offset_hour_i, "*", 1, mrb_fixnum_value(3600));
    mrb_value offset_min_i_sec =
        mrb_funcall(mrb, offset_min_i, "*", 1, mrb_fixnum_value(60));

    mrb_value total_offset =
        mrb_funcall(mrb, offset_hour_i_sec, "+", 1, offset_min_i_sec);
    mrb_value time_int = mrb_funcall(mrb, time_int_without_offset,
                                     RSTRING_PTR(ope), 1, total_offset);
...
static mrb_value strptime_to_i_without_offset_method(mrb_state *mrb,
                                                     mrb_value self) {
  mrb_value time = strptime_to_time_method(mrb, self);
...
void mrb_mruby_strptime_gem_init(mrb_state *mrb) {
  struct RClass *strptime_class =
      mrb_define_class(mrb, "Strptime", mrb->object_class);
  mrb_define_class_method(mrb, strptime_class, "parse", parse_method,
                          MRB_ARGS_REQ(1));
  mrb_define_method(mrb, strptime_class, "initialize", strptime_initialize,
                    MRB_ARGS_REQ(1));
  mrb_define_method(mrb, strptime_class, "to_time", strptime_to_time_method,
                    MRB_ARGS_NONE());
  mrb_define_method(mrb, strptime_class, "to_i", strptime_to_i_method,
                    MRB_ARGS_NONE());
  mrb_define_method(mrb, strptime_class, "to_i_without_offset",
                    strptime_to_i_without_offset_method, MRB_ARGS_NONE());
}

void mrb_mruby_strptime_gem_final(mrb_state *mrb) { /* finalizer */ }
